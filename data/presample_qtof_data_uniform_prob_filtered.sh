...
mkdir -p $DATA_DIR"/processed/data/presampled_qtof_basic_training_data_filtered/logs"
mkdir -p $DATA_DIR"/processed/data/presampled_qtof_basic_validation_data_filtered/logs"
...
bash nextflow run presample_and_assemble_data.nf --train_path $DATA_DIR"/processed/data/ALL_GNPS_positive_train_split.pickle" \
                                                 ...
                                                 --save_dir $DATA_DIR"/processed/data/presampled_qtof_basic_training_data_filtered" \
                                                 --num_epochs 1800 \
                                                 --memory_efficent true \
                                                 --mass_analyzer_lst "qtof" \
                                                 -c nextflow.config
...
bash nextflow run presample_and_assemble_data.nf --train_path $DATA_DIR"/processed/data/ALL_GNPS_positive_val_split.pickle" \
                                                 --tanimoto_scores_path $DATA_DIR"/processed/train_tanimoto_df.csv" \
                                                 --save_dir $DATA_DIR"/processed/data/presampled_qtof_basic_validation_data_filtered" \
                                                 --num_epochs 1 \
                                                 --num_turns 10 \
                                                 --memory_efficent true \
