...
def generate_pairs(spectra:list, 
    ...
    # Create a hdf file to save intermediate results to
    global_index = 0
    try:
        with tempfile.NamedTemporaryFile(suffix='.h5', delete=False) as temp_file:
            temp_filename = temp_file.name
            store = pd.HDFStore(temp_filename)
        ...
        for _, group in tqdm(spectrum_df):
            ...
            max_join_size = 1_000
            indices = np.arange(0, len(group), max_join_size)

            for subgroup_idx in tqdm(range(len(indices))):
                if subgroup_idx < len(indices) - 1:
                    subgroup = group.iloc[indices[subgroup_idx]:indices[subgroup_idx+1]]
                else:
                    subgroup = group.iloc[indices[subgroup_idx]:]
                if len(subgroup) == 0:
                ...
                store.put(f'pairs_{global_index}', pairs[['spectrum_id_1', 'spectrum_id_2', 'ms_mass_analyzer', 'ms_ionisation', 'precursor_mass_difference', 'inchikey_1', 'inchikey_2']], format='table', data_columns=True)
                global_index += 1
                del pairs
                gc.collect()
        ...
        # Concatenate all tables in the store and move to a dataframe
        df_list = []
        for key in store.keys():
            # Read the DataFrame for the current key and append to the list
            df_list.append(store[key])
