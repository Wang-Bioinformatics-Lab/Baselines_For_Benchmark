...
from ms2deepscore.MetadataFeatureGenerator import (MetadataFeatureGenerator,
                                                   load_from_json)
from ms2deepscore.BinnedSpectrum import BinnedSpectrum
from ms2deepscore.spectrum_binning_fixed import (create_peak_list_fixed, set_d_bins_fixed,
                                     unique_peaks_fixed)
...
class SpectrumBinner:
    ...
    def transform(self, input_spectrums: List[SpectrumType],
        ...
        """Create binned spectrums from input spectrums. This is a customized method
        that includes a `spectrum_id` for the binned spectrum.
        ...
        for i, peak_list in enumerate(tqdm(peak_lists,
            ...
            spectrum = BinnedSpectrum(binned_peaks=create_peak_dict(peak_list),
                                      metadata={"inchikey": input_spectrums[i].get("inchikey"), 
                                                "spectrum_id": input_spectrums[i].get("spectrum_id"),
                                                **additional_metadata})
